...
class LocalSystemServer:
    ...
    def _register_handlers(self) -> None:
        ...
        @self.server.call_tool() # Decorator applied to inner function
        ...
        @self.server.list_tools() # Decorator applied to inner function
    ...
    async def execute_command(
        self, arguments: dict[str, Any] # Removed request param
    ) -> list[TextContent]:
        ...
        if len(command) > self.security.security_config.max_command_length:
            raise ValueError(
                f"Command exceeds maximum length of {self.security.security_config.max_command_length}"
            )
    ...
    async def change_directory(
        ...
        return [
            TextContent(type="text", text=f"Changed directory to: {result.get('path')}")
        ]
    ...
    async def write_file(
        ...
        return [
            TextContent(type="text", text=f"File written successfully: {result.get('path')}")
        ]
    ...
    async def search_files(
        ...
        if not matches:
             return [TextContent(type="text", text=f"No files found matching pattern '{pattern}' in {normalized_path}")]
    ...
    async def directory_tree(
        ...
        if not tree: # Should not happen if success is true, but good practice
            return [TextContent(type="text", text=f"Directory is empty or inaccessible: {normalized_path}")]
    ...
    async def show_security_rules(
        ...
        # Exception handling remains
        commands_desc = (
            "All commands allowed" if self.security.security_config.allow_all_commands
            else ", ".join(sorted(self.security.security_config.allowed_commands))
        )
        flags_desc = (
            "All flags allowed" if self.security.security_config.allow_all_flags
    ...
    async def list_tools_impl(self) -> list[dict[str, Any]]:
        ...
        commands_desc = (
            "all commands" if self.security.security_config.allow_all_commands
            else ", ".join(self.security.security_config.allowed_commands)
        )
        flags_desc = (
            "all flags" if self.security.security_config.allow_all_flags
